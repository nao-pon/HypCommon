 ...
 *
 * $Id: mb-emulator.php,v 1.1 2007/11/20 09:00:10 nao-pon Exp $
 */
...
Class HypMBString
{
	...
	function mb_internal_encoding($encoding = '')
	{
	  if ($encoding =='') {
	    if ($this->mbemu_internals['internal_encoding'] == '') return FALSE;
			...
			$this->mbemu_internals['internal_encoding'] = $encoding;
			return TRUE;
	  }
	}
	...
	function mb_detect_encoding( $str , $encoding_list = '')
	{
		...
		foreach($encoding_list as $encode) {
			...
			if ($this->_check_encoding($str, $this->mbemu_internals['encoding'][$encode])) {
				return $encode;
			}
		}
